...
//    output e_signal);
    ...
    wire MemtoReg, MemWrite, ALU_Src, RegWrite, ecall, S_Type, BEQ, BNE, jal, jalr; // Output signals from the hard-wired controller
...
//    assign e_signal=equal;
    ...
    hard_wire_controller controller(.Funct(Funct), .OpCode(OpCode), .ALUOP(ALUOP), .MemtoReg(MemtoReg), .MemWrite(MemWrite),
        ...
        .jal(jal), .jalr(jalr)); // Hard-wired controller
    ...
    MEM Mem(.Addr(Result1[11:2]), .Din(R2), .CLK(CLK_N), .MemWrite(MemWrite), .sel(0), .Dout(MEMout)); // Data memory
    ...
    mux2 #(32) mux2_7(.out(PC_in), .in0(PC_Adr), .in1(Result1), .sel(Jalr_signal)); // 2-way multiplexer 7
